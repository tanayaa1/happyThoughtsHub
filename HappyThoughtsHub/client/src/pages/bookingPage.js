...
const BookingsPage = () => {
  ...
  return (
	...
	<div class="cta__text-column">
          <h2 className='bl1'>{selectedDoctor.doctorName}</h2>
          ...
          {selectedDate && (
            <div>
              <h3 className='bl1'>Select Time:</h3>
              <select onChange={(e) => handleTimeSelect(e.target.value)} value={selectedTime}>
                <option value="" disabled className='bl1'>Select a time</option>
                {getAvailableTimeSlots().map((time) => (
                  <option key={time} value={time}>
                    {time}
                  </option>
      ...
      <button onClick={handleConfirmAppointment} className='appointbut'>Confirm Appointment</button>
      {isAppointmentConfirmed && <p>Appointment Confirmed! Thank you.</p>}
